...
func (s *Server) appendJobLog(jobID, message string) {
	...
	now := time.Now().UTC()
	job.Stage = message; job.UpdatedAt = now
	if progress, ok := parseProgress(message); ok && progress > job.Progress { job.Progress = progress }

	job.Logs = append(job.Logs, schemas.JobEvent{Time: now, Message: message})
	if len(job.Logs) > 50 { job.Logs = append([]schemas.JobEvent(nil), job.Logs[len(job.Logs)-50:]...) }
...
func (s *Server) updateJob(jobID, status, stage string, progress float64, output interface{}, errStr string) {
	...
	job.Output = output; job.UpdatedAt = now; job.Artifacts = collectArtifacts(output)
	if errStr != "" { job.Error = errStr } else { job.Error = "" }
...
func parseProgress(message string) (float64, bool) {
	...
	if len(matches) != 3 { return 0, false }
	var current, total int
	if _, err := fmt.Sscanf(matches[0], "step %d/%d", &current, &total); err != nil || total <= 0 { return 0, false }
...
func collectArtifacts(output interface{}) []schemas.JobArtifact {
	...
	add := func(label, kind, path string) { if path != "" { artifacts = append(artifacts, schemas.JobArtifact{Label: label, Kind: kind, Path: path}) } }

	switch result := output.(type) {
	case *schemas.ImageResult: add("image", "image", result.ImagePath)
	case *schemas.ClipResult: add("image", "image", result.ImagePath); add("video", "video", result.VideoPath)
	case *schemas.PlanResult: add("plan", "json", result.PlanPath); add("narration_text", "text", result.NarrationPath)
	case *schemas.VoiceResult: add("voice", "audio", result.OutputPath)
	case *schemas.MusicResult: add("music", "audio", result.OutputPath)
...
func (s *Server) jobFilePath(jobID string) string { return filepath.Join(s.outputDir, jobID, "job.json") }
...
func (s *Server) persistJobLocked(job *schemas.Job) {
	...
	if err != nil { log.Printf("failed to marshal job %s: %v", job.JobID, err); return }
	if err := os.WriteFile(p, data, 0644); err != nil { log.Printf("failed to write job %s: %v", job.JobID, err) }
}
...
func defaultStr(val, def string) string { if val == "" { return def }; return val }

func defaultInt(val, def int) int { if val == 0 { return def }; return val }
